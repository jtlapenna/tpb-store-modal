...
(function($) {
    ...
    const Modal = {
        ...
        open: function(url) {
            ...
            // Add data attribute for modal type-specific styling and update content
            ...
            } else if (url.includes('quick-checkout-configure-now') || url.includes('quick-checkout-station-configure-now')) {
                $modal.attr('data-modal-type', 'quick-checkout');
                
                // Update Quick Checkout modal content
                const $title = $modal.find('.tpb-qv-title');
                const $description = $modal.find('.tpb-qv-base-price');
                
                if ($title.length) {
                    $title.text('Choose Your Quickcheckout Stations');
                    console.log('✅ Quick Checkout title updated');
                }
                
                if ($description.length) {
                    $description.html('Quick checkout stations provide our awesome shopping user interface without the physical product interaction. Mounting and furniture choices are subject to the designs of your store, and purchased separately from the electronics hardware-only kits below.');
                    console.log('✅ Quick Checkout description updated');
                }
            }

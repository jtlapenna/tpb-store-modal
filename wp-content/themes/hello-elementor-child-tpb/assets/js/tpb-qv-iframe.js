...
(function(window, document) {
    ...
    function hideComponent(comp) {
        if (!comp) return;
        // purpose: hide a CPB component completely
        try { console.log('📦 Hiding component:', (comp.querySelector('.title') || {}).textContent || 'Unknown'); } catch(_){}
    ...
    function showComponent(comp) {
        if (!comp) return;
        // purpose: show a CPB component
        try { console.log('📦 Showing component:', (comp.querySelector('.title') || {}).textContent || 'Unknown'); } catch(_){}
    ...
    function establishInitialFlowState() {
        ...
        components.forEach((comp, index) => {
            ...
            const title = (titleEl && titleEl.textContent ? titleEl.textContent.trim() : `Component ${index + 1}`);
            if (index === 0) {
    ...
    function setupFirstComponent(comp) {
        ...
        if (select) {
            setupSelectWithPlaceholder(select, comp);
        } else {
    ...
    function setupSelectWithPlaceholder(select, comp) {

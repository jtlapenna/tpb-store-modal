...
if ( ! class_exists( 'YITH_YWRAQ_Send_Email_Request_Quote_Customer' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class YITH_YWRAQ_Send_Email_Request_Quote_Customer extends YITH_YWRAQ_Email {
		/**
		...
		public function get_raq() {
			...
			if ( $this->is_email_preview() ) {
				// Prepare the placeholders for the preview from the quote data.
				$this->placeholders['{quote_number}']  = $raq['order-id'];
				$this->placeholders['{quote_user}']    = $raq['user_name'];
				$this->placeholders['{quote_email}']   = $raq['user_email'];
				$this->placeholders['{customer_name}'] = $raq['user_name'];
			}
			return $raq;
		}
	}

...
if ( ! class_exists( 'YITH_YWRAQ_Exclusions_Handler' ) ) {
	...
	class YITH_YWRAQ_Exclusions_Handler {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_ywraq_delete_from_exclusion_list', array( $this, 'handle_delete_from_exclusion_list' ) );
			//add_action( 'wp_ajax_nopriv_ywraq_delete_from_exclusion_list', array( $this, 'handle_delete_from_exclusion_list' ) );
		...
		public function handle_save_exclusion() {
			...
			if ( ! isset( $posted['_nonce'], $posted['ywraq-exclusion-type'] ) || ! wp_verify_nonce( $posted['_nonce'], 'yith_ywraq_add_exclusions' ) ) {
				return;
			}

			switch ( $posted['ywraq-exclusion-type'] ) {
				case 'product':
		...
		public function handle_delete_from_exclusion_list() {
			...
			if ( ! isset( $posted['nonce'], $posted['type'], $posted['id'] ) || ! wp_verify_nonce( $posted['nonce'], 'yith_ywraq_delete_exclusions' ) ) {
				return;
			}
		...
		public function save_exclusions_prod() {
			...
			update_option( 'yith-ywraq-exclusions-prod-list', $exclusions, 'no' );
		}
		...
		public function delete_exclusion_prod() {
			...
			update_option( 'yith-ywraq-exclusions-prod-list', implode( ',', $exclusions ), 'no' );
		}
		...
		public function save_exclusions_cat() {
			...
			update_option( 'yith-ywraq-exclusions-cat-list', implode( ',', $exclusions ), 'no' );
		}
		...
		public function save_exclusions_tag() {
			...
			// merge old with new.
			$exclusions = array_merge( $old_items, $new_items );
			update_option( 'yith-ywraq-exclusions-tag-list', implode( ',', $exclusions ), 'no' );
		}
 ...
 * @since 2.0.0
 */
function YITH_YWRAQ_Exclusions_Handler() { //phpcs:ignore

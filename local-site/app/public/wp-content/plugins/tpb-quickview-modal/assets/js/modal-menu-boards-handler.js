/**
 * TPB Menu Boards Modal Handler (v2.4, stepper-less)
 * - Integrates with TPB_QV modal-state v2.2 (open/close, focus, scroll-lock)
 * - Idempotent trigger wiring, scoped to the menu-boards overlay
...
(function ($, win, doc) {
  ...
  const LOG_PREFIX = '📋 [TPB MenuBoards]';
  const CFG   = win.TPB_QV_CONFIG || {};
  const DEBUG = !!CFG.debug;

  // ✅ FIXED: matches CONFIG keys and data-modal-type (singular)
  const MODAL_TYPE = 'menu-board';
  const OVERLAY_ID = 'tpb-qv-menu-boards-modal';
  ...
  const FALLBACK_URL_PLUGIN = PLUGIN_BASE + 'assets/img/menu-boards-hero.jpg';

  const SELECTORS = {
    overlay:         '#tpb-qv-menu-boards-modal',
    ...
    // Triggers (support both singular/plural slugs)
    triggers: [
      '[data-tpb-modal="menu-boards"]',
      '[data-tpb-quickview="menu-boards"]',
      '[data-tpb-modal="menu-board"]',
      '[data-tpb-quickview="menu-board"]',
      'a[href*="menu-boards-configure-now"]',
      'a[href*="menu-board-configure-now"]',
      'a[href*="/menu-boards"]',
      'a[href*="/menu-board"]',
      'a[href*="/configure-menu-boards"]',
      'a[href*="/configure-menu-board"]',
      'a[href*="/product/menu-boards"]',
      'a[href*="/product/menu-board"]'
    ].join(', ')
  };

  // Guard against double-wiring
  if (win.__TPB_Menu_Wired__) {
    if (DEBUG) console.debug(LOG_PREFIX, 'already wired; skipping re-init');
    return;
  }
  win.__TPB_Menu_Wired__ = true;
  ...
  function setImageInto($container, url, alt) {
    ...
    const $img = $('<img>').attr('src', url).attr('alt', alt || 'Menu Boards');
    ...
    return new Promise((resolve, reject) => {
      ...
      $.ajax({
        ...
        if (res && res.success && res.data && res.data.image_url) {
          resolve({ url: res.data.image_url, alt: res.data.image_alt || 'Menu Boards' });
        } else {
  ...
  function chooseAndRenderImage(productId) {
    ...
    if (cache && (productId || productId === 0)) {
      ...
      if (pre && pre.url) {
        ...
        setImageInto($image, pre.url, pre.alt || 'Menu Boards');
        return;
      }
    }
    ...
    if (productId) {
      fetchImageViaAjax(productId)
        ...
        .catch((e) => {
          ...
          setImageInto($image, fallback, 'Menu Boards');
        });
      return;
    }
  ...
  function openModalForTrigger($trigger) {
    ...
    if (win.TPB_QV && typeof win.TPB_QV.openModal === 'function') {
      ...
      if (typeof win.TPB_QV.emit === 'function') {
        win.TPB_QV.emit('menu-boards:open', { productId });
      }
    } else {
      // Fallback visibility
  ...
  function closeModal() {
    ...
    if (win.TPB_QV && typeof win.TPB_QV.closeModal === 'function') {
      ...
      if (typeof win.TPB_QV.emit === 'function') {
        win.TPB_QV.emit('menu-boards:close', {});
      }
    } else {
  ...
  function wireTriggers() {
    $(doc).on('click', SELECTORS.triggers, function (e) {
      ...
      // Only intercept when clearly a menu-boards target (supports singular/plural)
      if (!href ||
          href.indexOf('menu-boards') !== -1 ||
          href.indexOf('menu-board') !== -1 ||
          $t.data('tpbModal') === 'menu-boards' ||
          $t.data('tpbModal') === 'menu-board' ||
          $t.data('tpbQuickview') === 'menu-boards' ||
          $t.data('tpbQuickview') === 'menu-board') {
        e.preventDefault();
        e.stopPropagation();
  ...
  $(function init() {
    ...
    win.TPBMenuBoardsModal = win.TPBMenuBoardsModal || {};
    win.TPBMenuBoardsModal.open  = function () { openModalForTrigger($(SELECTORS.triggers).first()); };
    win.TPBMenuBoardsModal.close = closeModal;
  });

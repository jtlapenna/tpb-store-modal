...
class RateNotice implements Hookable {
	...
	public function maybe_reset_orders_counter( $notice_name, $source = null ) {
		if ( 'ups_rating' === $notice_name && ( empty( $source ) || self::CLOSE_TEMPORARY_NOTICE === $source ) ) {
			...
			delete_option( \UpsProVendor\WPDesk\Notice\PermanentDismissibleNotice::OPTION_NAME_PREFIX . $notice_name );
		}
	}
	...
	private function get_notice_content() {
		$content  = __( 'Awesome, you just crossed the 100 orders with UPS rates by Flexible Shipping UPS. Could you please do me a BIG favor and give it a 5-star rating on WordPress? ~ Peter', 'flexible-shipping-ups-pro' );
		$content .= '<br/>';
	...
	public function maybe_show_notice() {
		...
		if ( self::FIRST_NOTICE_MIN_ORDERS <= $order_counter && $this->should_display_notice() ) {
			new \UpsProVendor\WPDesk\Notice\PermanentDismissibleNotice(

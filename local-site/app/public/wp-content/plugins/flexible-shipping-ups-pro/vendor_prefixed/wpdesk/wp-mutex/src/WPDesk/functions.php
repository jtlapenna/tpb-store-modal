...
{
    return new \UpsProVendor\WPDesk\Mutex\WordpressMySQLLockMutex($lockName, $waitForLockTimeout);
}
/**
...
{
    return \UpsProVendor\WPDesk\Mutex\WordpressMySQLLockMutex::fromOrder($order, $lockName, $waitForLockTimeout);
}
/**
 * Acquire lock.
 *
...
function wpdesk_acquire_lock($lockName, $waitForLockTimeout = 5, $lockType = 'mysql')
{
    if ('mysql' === $lockType) {
        ...
        $storage = new \UpsProVendor\WPDesk\Mutex\StaticMutexStorage();
...
function wpdesk_release_lock($lockNAme)
{
    ...
    if (null !== $mutex) {
        ...
        throw new \UpsProVendor\WPDesk\Mutex\MutexNotFoundInStorage();
    }
}

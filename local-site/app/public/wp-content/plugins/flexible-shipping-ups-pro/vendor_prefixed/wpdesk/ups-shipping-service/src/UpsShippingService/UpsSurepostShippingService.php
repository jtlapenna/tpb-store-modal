...
use UpsProVendor\WPDesk\UpsShippingService\UpsApi\UpsSenderSingleRate;
/**
...
class UpsSurepostShippingService extends AbstractUpsShippingService
{
    ...
    protected function create_single_rate_sender(SettingsValues $settings, string $surepost_service_code)
    {
        if ($settings->get_value(UpsSettingsDefinition::API_TYPE, UpsSettingsDefinition::API_TYPE_XML) === UpsSettingsDefinition::API_TYPE_REST) {
            return new \UpsProVendor\Octolize\Ups\RestApi\UpsRestApiSenderSingleRate($this->rest_api_client, $surepost_service_code, $this->get_logger(), $this->is_testing($settings), $this->get_shop_settings()->is_tax_enabled());
        } else {
    ...
    {
        return __('UPS Ground Saver Live Rates', 'flexible-shipping-ups-pro');
    }
    /**
     * Get description.
     *

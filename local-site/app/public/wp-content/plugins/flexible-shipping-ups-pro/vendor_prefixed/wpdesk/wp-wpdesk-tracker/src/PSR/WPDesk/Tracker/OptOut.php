...
namespace UpsProVendor\WPDesk\Tracker;
    ...
    public function handle_opt_out()
    {
        ...
        if ('plugins' === $screen->id && isset($_GET['wpdesk_tracker_opt_out_' . $this->plugin_slug])) {
            ...
            if (wp_verify_nonce($security, $this->plugin_slug)) {
                $persistence = new \UpsProVendor\WPDesk_Tracker_Persistence_Consent();
                $persistence->set_active(\false);
                delete_option('wpdesk_tracker_notice');
                new Notice(sprintf(esc_html__('You successfully opted out of collecting usage data by %1$s. If you change your mind, you can always opt in later in the plugin\'s quick links.', 'flexible-shipping-ups-pro'), esc_html($this->plugin_name)));
            }
        }
    }
}

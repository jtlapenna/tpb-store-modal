...
namespace UpsProVendor\WPDesk\License\LicenseServer;
...
class PluginUpgrade
{
    ...
    private function react_to_wp_error_response(\WP_Error $remote_response): \stdClass
    {
        ...
        $response->message = wp_kses_post('<span style="color: red" class="error">' . sprintf(__('Error while connecting to remote server %s. Please contact your hosting provider or try again later. Errors:  ', 'flexible-shipping-ups-pro'), $this->server) . implode(', ', $remote_response->get_error_messages()) . '</span>');
        return $response;
    }
    /**
    ...
    private function render_changelog(string $plugin_version, string $changelog): string
    {
        ...
        if (iterator_count($changes) > 0) {
            $changelogFormatter = new \UpsProVendor\WPDesk\License\Changelog\Formatter($changes);
    ...
    private function react_to_nonsense_response($remote_response): \stdClass
    {
        ...
        if (isset($remote_response['response']['code'])) {
            $message = sprintf(__('Error while connecting to remote server %s. Please contact with your hosting provider. Cannot parse response. Response code: %s Message: %s', 'flexible-shipping-ups-pro'), $this->server, $remote_response['response']['code'], $remote_response['body']);
        } else {
            $message = sprintf(__('Error while connecting to remote server %s. Please contact with your hosting provider. Cannot parse response: %s', 'flexible-shipping-ups-pro'), $this->server, json_encode($remote_response));
        }

...
use UpsProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RulesTableAdv;
/**
    ...
    private function replace_fallback_field_if_exists($settings)
    {
        ...
        foreach ($settings as $key => $field) {
            if ($field['type'] === FallbackRateMethod::FIELD_TYPE_FALLBACK) {
                $new_settings[FallbackRateMethod::FIELD_ENABLE_FALLBACK] = ['title' => __('Fallback', 'flexible-shipping-ups-pro'), 'type' => 'checkbox', 'label' => __('Enable fallback', 'flexible-shipping-ups-pro'), 'description' => __('Enable to offer flat rate cost for shipping so that the user can still checkout, if API for some reason returns no matching rates.', 'flexible-shipping-ups-pro'), 'desc_tip' => \true, 'default' => 'no'];
                $new_settings[FallbackRateMethod::FIELD_FALLBACK_COST] = ['title' => __('Fallback Cost', 'flexible-shipping-ups-pro'), 'type' => 'price', 'required' => \true, 'description' => __('Enter only a numeric value without the currency symbol.', 'flexible-shipping-ups-pro'), 'desc_tip' => \true, 'default' => ''];
            } else {

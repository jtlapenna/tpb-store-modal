...
class RateExceptionWithExtendedInfo extends RateException
{
    ...
    private function extendMessage(string $message, int $code)
    {
        if (in_array($code, [111035, 111036], \true)) {
            ...
            $message .= sprintf(__('%1$sTo calculate rates using multiple boxes, use "Pack items separately" or "Pack into custom boxes". Learn more here: %2$soctolize.com/docs%3$s', 'flexible-shipping-ups-pro'), ' ' . \PHP_EOL, '<a href="' . $url . '" target="_blank">', '</a>');
        }
        return $message;
    }
}

...
namespace UpsProVendor\Octolize\WooCommerceShipping\Ups\OAuth;
...
class OAuthField
{
    ...
    public function __construct(string $key, array $data, string $field_key, TokenOption $token_option, string $settings_url, string $oauth_url, string $app = 'live_rates', string $test_api = '')
    {
        $this->key = $key;
        $this->data = $data;
    ...
    public function generate_oauth_html(): string
    {
        ...
        $revoke_action = admin_url(sprintf('admin-ajax.php?action=%s&security=%s', urlencode(Ajax::ajax_action_name($this->token_option)), urlencode($nonce)));

...
use UpsProVendor\GuzzleHttp\Client;
use UpsProVendor\GuzzleHttp\Exception\ClientException;
use UpsProVendor\Psr\Http\Message\ResponseInterface;
...
class RestApiClient implements LoggerAwareInterface
{
    ...
    private function throw_exception_from_response(ResponseInterface $response): void
    {
        ...
        if (!is_array($body) && !empty($body)) {
            throw new RestApiException(sprintf(__('Invalid response: %s', 'flexible-shipping-ups-pro'), print_r($body, \true)), $response->getStatusCode());
        }

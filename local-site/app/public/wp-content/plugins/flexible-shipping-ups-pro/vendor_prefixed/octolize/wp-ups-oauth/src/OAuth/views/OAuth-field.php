...
if (empty($token_option->get_refresh_token())) {
    ...
    ?>"
			...
			><?php 
    echo \esc_html(\__('Authorize', 'flexible-shipping-ups-pro'));
    ?></button>
			<p class="description">
				<?php 
    echo \esc_html(\__('Clicking the button will open up the UPS website. Please provide your credentials and mark the checkbox to connect our plugin with your UPS account.', 'flexible-shipping-ups-pro'));
    ?>
			</p>
		<?php 
} else {
    ...
    ?>"
			...
			><?php 
    echo \esc_html(\__('Revoke', 'flexible-shipping-ups-pro'));
    ?></button>
			<p class="description">
				<?php 
    echo \esc_html(\__('Clicking the button will disconnect your UPS account from our plugin.', 'flexible-shipping-ups-pro'));
    ?>
				<!-- Expires at: <?php 
		...
		<script>
			jQuery(document).ready(function () {
				...
				jQuery(document).on('click', '.ups-oauth-button', function (e) {
					if (changed) {
						alert('<?php 
echo \esc_js(\__('You have unsaved changes. Save them first.', 'flexible-shipping-ups-pro'));
?>');
					} else {

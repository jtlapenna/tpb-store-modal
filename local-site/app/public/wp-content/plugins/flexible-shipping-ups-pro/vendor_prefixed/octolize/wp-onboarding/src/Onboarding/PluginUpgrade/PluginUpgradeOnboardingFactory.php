...
namespace UpsProVendor\Octolize\Onboarding\PluginUpgrade;

use UpsProVendor\Octolize\Onboarding\Field\Html;
use UpsProVendor\Octolize\Onboarding\Onboarding;
use UpsProVendor\Octolize\Onboarding\OnboardingButton;
use UpsProVendor\Octolize\Onboarding\OnboardingOption;
use UpsProVendor\Octolize\Onboarding\OnboardingShouldShowAlwaysStrategy;
use UpsProVendor\Octolize\Onboarding\OnboardingShouldShowStrategy;
use UpsProVendor\Octolize\Onboarding\OnboardingStep;
use UpsProVendor\Octolize\Onboarding\OnboardingTrackerData;
use UpsProVendor\WPDesk\Forms\Field;
/**
...
class PluginUpgradeOnboardingFactory
{
    ...
    private function prepare_steps(string $previous_version, string $current_version): array
    {
        ...
        $onboarding_step->set_show(\true)->set_heading(sprintf(__('Thank you for updating %1$s!', 'flexible-shipping-ups-pro'), $this->plugin_name))->set_sub_heading(__('It is really important to keep the plugins up to date. We have implemented some improvements and new functionalities. Find out what has changed:', 'flexible-shipping-ups-pro'));
    ...
    {
        return [(new OnboardingButton())->set_label(__('I\'m not interested', 'flexible-shipping-ups-pro'))->set_classes(OnboardingButton::BTN_LINK), (new OnboardingButton())->set_label(__('Thanks for letting me know', 'flexible-shipping-ups-pro'))->set_type(OnboardingButton::TYPE_CLOSE)];
    }

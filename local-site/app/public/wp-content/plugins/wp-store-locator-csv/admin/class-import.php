...
if ( !class_exists( 'WPSL_CSV_Import' ) ) {
	...
	class WPSL_CSV_Import {
        ...
        /**
         * Make sure the geocode class from the WPSL plugin is included.
         *
         * @since 1.2.12
         * @note Fixes rare issues where after WP 5.8.1
         * it seemingly randomly can't access the
         * 'WPSL_Geocode' class.
         *
         * Only happens for a few users, and not sure why
         * this suddenly happend, but this fixes it.
         */
        public function includes() {
            require_once( WP_PLUGIN_DIR . '/wp-store-locator/admin/class-geocode.php' );
        }
            ...
            foreach ( $add_locations as $store_location ) {
                ...
                if ( !strlen( trim( $wpsl_id ) ) ) {
                    ...
                    if ( get_post_type( $wpsl_id ) == 'wpsl_stores' ) {
                        ...
                        $post_id = new WP_Error( 'invalid_id', sprintf( __( 'Update failed! The provided wpsl_id %s doesn\'t belong to a store location. You can fix this by setting the mapping for the wpsl_id field to unmapped, or make sure the wpsl_id value belongs to an existing location.', 'wpsl-csv' ), $store_location['wpsl_id'] ) );
                    }
                } else {
                    $post_id  = '';
                }
